...
- [x] Ignore same-game tool notifications while a direct GPT mutation is pending, except only a field-for-field exact lifecycle end/reset transition; manual end requires unchanged board, history, last move, and all other game data apart from status, winner, finish reason, legal moves, state version, and message.
- [x] Treat `MOVE_NOT_APPLIED`, validation, and version failures as definite: report them without a state read or a second mutation.
- [x] Treat transport or mismatched-confirmation outcomes as ambiguous: read state once, accept only the matching exact advance, otherwise show the safe Refresh error.
...
- [x] Focused `web/src/App.test.tsx`: 60 passing tests, including direct embedded success with one selected move/no message/no state read, exact receipt unlock, altered-history, altered-ended, and injected-notification rejection, one-read ambiguity recovery/no retry, mismatched one-read receipt rejection, definite rejection/no read/no retry, Go review gating, repeated GPT turns, and End/Reset interruption with late-receipt invalidation.
- [x] Full web suite: 120 passing tests across 7 files.
