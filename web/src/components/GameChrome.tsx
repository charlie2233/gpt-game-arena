...
export function GameChrome({ game }: { game: GameSnapshot }) {
  ...
  return <><section className="roles" aria-label="Game roles"><div><span className="token player-token">{mark(game.playerColor)}</span><strong>You are {title(game.playerColor)}</strong></div><div className="turn" aria-live="polite"><i className={mine ? "dot mine" : "dot"}/><b>{game.status === "finished" ? "Game finished" : mine ? "Your turn" : "GPT thinking…"}</b><span>{game.message}</span></div><div><span className="token gpt-token">{mark(other(game.playerColor))}</span><span className="gpt-role"><strong>GPT is {title(other(game.playerColor))}</strong><small>{title(game.difficulty)} difficulty</small></span></div></section><aside className="history"><h2>Move history</h2><ol>{game.moveHistory.map(move => <li key={move.ply}><span>{move.ply % 2 ? `${Math.ceil(move.ply/2)}.` : `${Math.ceil(move.ply/2)}…`}</span>{move.notation}</li>)}</ol></aside></>;
}
