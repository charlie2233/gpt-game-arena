...
export function ConnectFourBoard({ game, onMove, disabled }: { game: ConnectFourSnapshot; onMove: (move: string) => void; disabled: boolean }) {
  const legal = new Set(game.legalMoves); const winners = new Set(game.winningLine);
  return <div className="connect-four-wrap"><div className="connect-actions" role="group" aria-label="Connect Four columns">{[..."ABCDEFG"].map(column => <button key={column} disabled={disabled || !legal.has(column as never)} aria-label={`Drop in column ${column}${legal.has(column as never) ? ", legal move" : ""}`} onClick={() => onMove(column)}>{column}</button>)}</div><div className="connect-board" role="grid" aria-label="Connect Four board">{game.board.map((row, r) => <div role="row" key={r}>{row.map((cell, c) => { const move = coordinate(r, c, 6); const winning = winners.has(move as never); return <span role="gridcell" aria-label={`${cell ? `${cell} disk` : "empty"} at ${move}${winning ? ", winning disk" : ""}`} key={`${r}-${c}`} className={`connect-cell${winning ? " winning" : ""}`}>{disk(cell)}</span>; })}</div>)}</div></div>;
}
