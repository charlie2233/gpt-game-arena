...
describe("authoritative snapshot validation", () => {
  ...
  it("accepts supported Go sizes and rejects mismatched or unsupported boards", () => {
    const go = (boardSize: number, rows = boardSize) => ({ gameId: "go", kind: "go", playerColor: "black", turn: "black", status: "active", legalMoves: ["pass"], moveHistory: [], stateVersion: 0, message: "ok", boardSize, board: Array.from({ length: rows }, () => Array.from({ length: boardSize }, () => null)), captures: { black: 0, white: 0 }, consecutivePasses: 0 });
