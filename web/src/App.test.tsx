...
import type { ChessSnapshot, GameDifficulty, GoBoardSize, GoSnapshot, ChessSquare, TicTacToeSnapshot, ConnectFourSnapshot, ReversiSnapshot } from "./types";
...
const tic = (version = 0): TicTacToeSnapshot => ({ gameId: "tic", kind: "tic-tac-toe", difficulty: "hard", playerColor: "black", turn: "black", status: "active", legalMoves: ["A3", "B2"], moveHistory: [], stateVersion: version, message: "Black to move.", board: Array.from({ length: 3 }, () => Array<"white" | "black" | null>(3).fill(null)) });
const four = (version = 0): ConnectFourSnapshot => ({ gameId: "four", kind: "connect-four", difficulty: "hard", playerColor: "black", turn: "black", status: "active", legalMoves: ["A", "D"], moveHistory: [], stateVersion: version, message: "Black to move.", board: Array.from({ length: 6 }, () => Array<"white" | "black" | null>(7).fill(null)) });
const reversi = (version = 0): ReversiSnapshot => ({ gameId: "rev", kind: "reversi", difficulty: "hard", playerColor: "black", turn: "black", status: "active", legalMoves: ["C4", "D3"], moveHistory: [], stateVersion: version, message: "Black to move.", board: Array.from({ length: 8 }, () => Array<"white" | "black" | null>(8).fill(null)), score: { black: 2, white: 2 } });
describe("App", () => {
  ...
  it("plays direct server-shaped Tic, Reversi, and column-based Connect Four moves and never exposes Pass", async () => {
    const cases: Array<[TicTacToeSnapshot | ConnectFourSnapshot | ReversiSnapshot, RegExp, string]> = [[tic(3), /A3, empty, legal move/, "A3"], [reversi(3), /C4, empty, legal move/, "C4"], [four(3), /Drop in column A/, "A"]];
    for (const [start, control, move] of cases) {
      cleanup(); vi.mocked(fetch).mockReset(); const after = { ...start, stateVersion: 4, turn: "white" as const, legalMoves: start.kind === "connect-four" ? ["D"] : start.kind === "tic-tac-toe" ? ["C3"] : ["D3"] }; const gpt = { ...after, stateVersion: 5, turn: "black" as const, legalMoves: start.legalMoves };
      ...
      render(<App initialGame={start}/>); await userEvent.setup().click(screen.getByRole("button", { name: control })); await waitFor(() => expect(fetch).toHaveBeenCalledTimes(2)); expect((vi.mocked(fetch).mock.calls[0][1] as RequestInit).body).toContain(`"move":"${move}"`); expect((vi.mocked(fetch).mock.calls[0][1] as RequestInit).body).toContain("\"expectedVersion\":3"); expect(screen.queryByRole("button", { name: /pass/i })).not.toBeInTheDocument();
    }
  });
  it("renders Reversi score, winning highlights, and deterministic text rows for every new board", () => {
    const finished: TicTacToeSnapshot = { ...tic(), status: "finished", winner: "black", winningLine: ["A3", "B3", "C3"], board: [["black", "black", "black"], [null, null, null], [null, null, null]] };
    render(<App initialGame={finished}/>); expect(screen.getByRole("button", { name: /A3, X/i })).toHaveClass("winning"); expect(JSON.parse(window.render_game_to_text!()).coordinateSystem).toContain("A-C"); cleanup(); render(<App initialGame={reversi()}/>); expect(screen.getByText("Disks — Black: 2, White: 2")).toBeVisible(); const revText = JSON.parse(window.render_game_to_text!()); expect(revText.coordinateSystem).toContain("A-H"); expect(revText.game.board).toHaveLength(8); cleanup(); render(<App initialGame={four()}/>); const fourText = JSON.parse(window.render_game_to_text!()); expect(fourText.coordinateSystem).toContain("A-G"); expect(fourText.game.board).toHaveLength(6);
  });
});
