...
export function App({ bridge: suppliedBridge, initialGame }: { bridge?: GameBridge; initialGame?: GameSnapshot } = {}) {
  ...
  useEffect(() => { const unsubscribe = bridge.onToolResult(result => { if (isSnapshot(result.structuredContent)) { stop(); setBusy(false); setSelected(undefined); setGame(result.structuredContent); } }); const context = bridge.onHostContext(() => undefined); if (bridge.embedded) void bridge.initialize().catch(() => setError("Could not initialize the game host.")); return () => { unsubscribe(); context(); stop(); bridge.dispose(); }; }, [bridge, stop]);
