...
export function App({ bridge: suppliedBridge, initialGame }: { bridge?: GameBridge; initialGame?: GameSnapshot } = {}) {
  ...
  useEffect(() => { let alive = true; const initEpoch = epoch.current; const unsubscribe = bridge.onToolResult(result => { const next = result.structuredContent; const current = gameRef.current; if (!isSnapshot(next)) return; if (!current) { stop(); commitBusy(false); gameRef.current = next; setGame(next); return; } if (next.gameId !== current.gameId) return; if (next.stateVersion === 0) { const ceiling = Math.max(resetBarrier.current?.gameId === next.gameId ? resetBarrier.current.ceiling : 0, current.stateVersion + 1); resetBarrier.current = { gameId: next.gameId, ceiling }; stop(); commitBusy(false); setSelected(undefined); gameRef.current = next; setGame(next); return; } if (resetBarrier.current?.gameId === next.gameId) { if (next.stateVersion <= resetBarrier.current.ceiling) return; resetBarrier.current = undefined; } if (busyRef.current || next.stateVersion <= current.stateVersion) return; gameRef.current = next; setGame(next); }); const context = bridge.onHostContext(() => undefined); if (bridge.embedded) void bridge.initialize().catch(() => { if (alive && epoch.current === initEpoch) setError("Could not initialize the game host."); }); return () => { alive = false; unsubscribe(); context(); stop(); bridge.dispose(); }; }, [bridge, commitBusy, stop]);
